...
- [How to enable WebAuthn / Passkey authentication?](https://miniflux.app/docs/howto.html#webauthn)
- [Use a Unix socket for Postgresql](https://miniflux.app/docs/howto.html#pg-unix-socket)
- [How to run Miniflux on port 443 or 80](https://miniflux.app/docs/howto.html#privileged-ports)
- [Reverse\-Proxy Configuration](https://miniflux.app/docs/howto.html#reverse-proxy)
- [Reverse\-Proxy with a subfolder](https://miniflux.app/docs/howto.html#reverse-proxy-subfolder)
- [Reverse\-Proxy with a Unix socket](https://miniflux.app/docs/howto.html#reverse-proxy-unix-socket)
- [Systemd Socket Activation](https://miniflux.app/docs/howto.html#systemd-socket-activation)
- [Let’s Encrypt Configuration](https://miniflux.app/docs/howto.html#lets-encrypt)
- [Manual HTTPS Configuration](https://miniflux.app/docs/howto.html#https)
- [OAuth2 and OpenID Connect Authentication](https://miniflux.app/docs/howto.html#oauth2)
- [Deploy Miniflux on Heroku](https://miniflux.app/docs/howto.html#heroku)
- [Deploy Miniflux on Google App Engine](https://miniflux.app/docs/howto.html#gae)

## How to enable WebAuthn / Passkey authentication? [¶](https://miniflux.app/docs/howto.html#webauthn)
- Define `BASE\_URL` with an appropriate value, for example: `https://miniflux.example.org/`. It won’t work with an IP address like `127.0.0.1` in some browsers.
...
Ports less than 1024 are reserved for privileged users. If you have installed Miniflux with the RPM or Debian package, no further action is required. Systemd runs the process as the `miniflux` user with `CAP\_NET\_BIND\_SERVICE` capability.

If you are not using systemd and want to give Miniflux the ability to bind to privileged ports as a non\-root user, add the capability `CAP\_NET\_BIND\_SERVICE` to the binary:
...
You must define the environment variable `BASE\_URL` for Miniflux, for example:
...
Place this snippet inside your vhosts config, needed modules: `mod\_proxy` and `mod\_proxy\_http`.

## Reverse\-Proxy with a Unix socket [¶](https://miniflux.app/docs/howto.html#reverse-proxy-unix-socket)

If you prefer to use a Unix socket, change the environment variable `LISTEN\_ADDR` to the path of your socket.
...
- When using this feature, Miniflux ignores `LISTEN\_ADDR`.
...
- In this mode, `LISTEN\_ADDR` is automatically set to `:https`
...
If you would like to authorize anyone to create a user account, you must set `OAUTH2\_USER\_CREATION=1`. Since Google do not have the concept of username, the email address is used as username.
...
- Since Miniflux 2.0.48, the `profile` scope will be requested in addition to the `email` scope. If no email address is available, Miniflux will fallback to the `preferred\_username` or `name` claims to populate Miniflux’s `username` field.
- Since Miniflux 2.2.10, if the `preferred\_username` claim is present and not empty, it is used as the username instead of the `email` claim.
...
Authentik discovery endpoint is `https://authentik.example.org/application/o/miniflux/.well\-known/openid\-configuration` and the `OAUTH2\_OIDC\_DISCOVERY\_ENDPOINT` config option should be `https://authentik.example.org/application/o/miniflux/` \(note the trailing `/`\).
...
The value of `OAUTH2\_OIDC\_DISCOVERY\_ENDPOINT` typically matches the `issuer` field in the response from `https://keycloak.example.org/realms/master/.well\-known/openid\-configuration`.
...
If you have experience with Terraform and want a setup that works out of the box, consider using this [Terraform module](https://github.com/huy-nguyen/terraform-google-miniflux). Alternatively, you can follow these instructions:
...
- [https://cloud.google.com/appengine/docs/standard/go111/building\-app/](https://cloud.google.com/appengine/docs/standard/go111/building-app/)
- [https://cloud.google.com/appengine/docs/standard/go111/using\-cloud\-sql](https://cloud.google.com/appengine/docs/standard/go111/using-cloud-sql)
Running Miniflux on Google App Engine should work but it's considered experimental.
[Edit this page](https://github.com/miniflux/website/edit/main/content/docs/howto.md)
